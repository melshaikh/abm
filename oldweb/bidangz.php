		...
		<body>
                ...
                ?>
                    <?php if(isset($_POST['createuser'])){
                        ...
                        $chk = "SELECT * FROM `area` WHERE `name` = '".$_POST['name']."'";
                        $ck = $db->query($chk);
                        if($ck->num_rows < 1){
                        if($db->query($sql)){
                    ...
                    <?php if(isset($_POST['newuserform'])){?>
                        ...
                        <div class="overlay">
                                <div class="container">
                                    <form action="bidangz.php" method="post">
                ...
                <!-- /contactWrap -->  
                    <?php } if(isset($_POST['editbidang'])){
                        ...
                        $sql = "UPDATE `area` SET `name` = '".$_POST['name']."' WHERE `area`.`id` = ".$_POST['bidid'];
                        $db->query($sql);
                        
                    }
                if(isset($_GET['edit'])){
                    $bid = getAreaByID($_GET['edit']);
                ?>
                   <div style="margin: 3%;">
                        <div class="overlay">
                                <div class="container">
                                    <form action="bidangz.php" method="post">
                                        <input type="text" name="name" value="<?php echo $bid['name'];?>"/>
                                        <input type="hidden" value="<?php echo $bid['id'];?>" name="bidid"/>
                                        <input type="submit" name="editbidang" value="CHANGE"/>
                                    </form>
                                </div>
                        </div>
                </div> <!-- /contactWrap --> 
                ...
                <div style="margin: 3%;">
                        <div class="overlay">
                                <div class="container">
                                    <form action="bidangz.php" method="post">
                                        <input type="submit" name="newuserform" value="create new bidang"/>
                                    </form>
                                </div>
                        </div>
                </div> <!-- /contactWrap --> 
                <div>
                        <div class="overlay">
                                <div class="container">
                                    <h1>List of Bidang </h1>
                                    <?php $userlist = getArea();?>
                                    <table class="table table-condensed table-striped table-bordered">
                                        <thead><tr><th>Name</th><th>Edit</th><th>Delete</th></tr></thead>
                                        <tbody>
                                        <tr>
                                            <?php   while ($us = $userlist->fetch_assoc()){ ?>
                                            <tr><td><?php echo $us['name']; ?></td>
                                                <td><a href="bidangz.php?edit=<?php echo $us['id']; ?>">EDIT</a></td>
                                                <td><a href="bidangz.php?del=<?php echo $us['id']; ?>">DELETE</a></td>
                                            <?php } ?>
                                        </tr>
                                        </tbody>
                                    </table>
